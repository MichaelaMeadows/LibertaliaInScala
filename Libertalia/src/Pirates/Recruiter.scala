...
class Recruiter(owningPlayer:Int) extends Pirate(4, owningPlayer) {
   ...
   */
   def dayActivity(state: GameState) {
     val piratesInDen = state.players(owningPlayer).getCardsInState(DEN);
     // You MUST place a card into your hand if able.
     if (piratesInDen.size > 0) {
         var decision:Int = state.players(owningPlayer).makeDecision(state, piratesInDen, "What pirate do you wish to return to your hand?");
         state.players(owningPlayer).getPirateFromDeck(decision).state = HAND;
     }
   }
